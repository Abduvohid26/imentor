"""PPTX/PPT → PDF preview (LibreOffice). FastAPI `pptx_preview` bilan bir xil mantiq."""
...
def ensure_presentation_preview_pdf(source: Path) -> Path:
    ...
    if not soffice:
        raise RuntimeError("LibreOffice o'rnatilmagan (soffice).")
    ...
    with tempfile.TemporaryDirectory(prefix="imentor-pptx-") as tmp:
        ...
        if proc.returncode != 0:
            logger.error("LibreOffice convert failed rc=%s stderr=%s", proc.returncode, (proc.stderr or "")[:800])
